...
def _iter_self_attn_linear_names(model):
...
def _iter_linear_names(model, name_predicate=None):
    for name, module in model.named_modules():
        ...
        if name_predicate is None:
            ...
            continue
        try:
            if bool(name_predicate(name, module)):
                yield name
        except Exception:
            continue


def _resolve_rank_for_module(module, r=None, residual_dim=None):
    k = int(min(module.in_features, module.out_features))
    if residual_dim is not None:
        rd = int(max(1, int(residual_dim)))
        return int(max(1, k - rd))
    rr = int(max(1, int(r if r is not None else k)))
    return int(min(rr, k))


def _finalize_trainable_flags(model, freeze_non_residual=True):
    if not freeze_non_residual:
        return model
...
def apply_svd_residual_to_linears_ex(
    model,
    r=None,
    residual_dim=None,
    name_predicate=None,
    ...
    show_progress=True,
    fast_init=False,
    freeze_non_residual=True,
):
    names = list(_iter_linear_names(model, name_predicate=name_predicate))
    iterator = names
    if show_progress:
        try:
    ...
    for name in iterator:
        ...
        if not isinstance(sub_module, nn.Linear):
            continue
        layer_r = _resolve_rank_for_module(sub_module, r=r, residual_dim=residual_dim)
        if fast_init:
            new_module = replace_with_svd_residual_fast(sub_module, layer_r)
        else:
            new_module = replace_with_svd_residual(sub_module, layer_r, svd_device=svd_device)
        setattr(parent, attr, new_module)

    return _finalize_trainable_flags(model, freeze_non_residual=freeze_non_residual)


def apply_svd_residual_to_self_attn_ex(model, r, svd_device=None, show_progress=True, fast_init=False):
    return apply_svd_residual_to_linears_ex(
        model=model,
        r=r,
        residual_dim=None,
        name_predicate=lambda name, module: ("self_attn" in name),
