...
Example:
  python full_submission.py\
      --ckpt /ssd2/ntire_RDFC_2026_cvpr/New-Effort-AIGI-Detection_dual/checkpoints/Effort_aug_pooling_baseline/epoch_1.pt\
          --txt_path /ssd2/ntire_RDFC_2026_cvpr/challange_data/validation_data_final\
              --root_dir /ssd2/ntire_RDFC_2026_cvpr/challange_data/validation_data_final\
...
DEFAULT_ROOT = Path("/ssd2/ntire_RDFC_2026_cvpr/challange_data/validation_data_final")
...
class TxtImageDataset(Dataset):
    def __init__(
        ...
        self.normalize_mean = _parse_rgb_triplet(normalize_mean, CLIP_MEAN)
        self.normalize_std = [max(1e-6, float(v)) for v in _parse_rgb_triplet(normalize_std, CLIP_STD)]
...
def _clip_stats(device, dtype, normalize_mean=None, normalize_std=None):
    mean_v = _parse_rgb_triplet(normalize_mean, CLIP_MEAN)
    std_v = [max(1e-6, float(v)) for v in _parse_rgb_triplet(normalize_std, CLIP_STD)]
    mean = torch.tensor(mean_v, device=device, dtype=dtype).view(1, 3, 1, 1)
    std = torch.tensor(std_v, device=device, dtype=dtype).view(1, 3, 1, 1)
    return mean, std


def _denorm_clip(images_norm: torch.Tensor, normalize_mean=None, normalize_std=None) -> torch.Tensor:
    mean, std = _clip_stats(images_norm.device, images_norm.dtype, normalize_mean=normalize_mean, normalize_std=normalize_std)
    return (images_norm * std + mean).clamp(0.0, 1.0)


def _norm_clip(images: torch.Tensor, normalize_mean=None, normalize_std=None) -> torch.Tensor:
    mean, std = _clip_stats(images.device, images.dtype, normalize_mean=normalize_mean, normalize_std=normalize_std)
...
def _tri_view_degrade(
    ...
    *,
    ...
    x = _denorm_clip(images_norm, normalize_mean=normalize_mean, normalize_std=normalize_std).float()
    ...
    return _norm_clip(x.to(dtype=images_norm.dtype), normalize_mean=normalize_mean, normalize_std=normalize_std)
...
def _tri_view_restore(
    ...
    *,
    ...
    x = _denorm_clip(images_norm_deg, normalize_mean=normalize_mean, normalize_std=normalize_std).float()
    ...
    if (original_norm is not None) and (mix > 0.0):
        xo = _denorm_clip(original_norm, normalize_mean=normalize_mean, normalize_std=normalize_std).float()
        x = ((1.0 - mix) * x + mix * xo).clamp(0.0, 1.0)

    return _norm_clip(x.to(dtype=images_norm_deg.dtype), normalize_mean=normalize_mean, normalize_std=normalize_std)


def _tta_resize_back(images_norm: torch.Tensor, scale: float, normalize_mean=None, normalize_std=None) -> torch.Tensor:
    scale = float(scale)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--svd_rank_cap", type=int, default=SVD_RANK_CAP)
    parser.add_argument("--adapter_last_n_layers", type=int, default=0)
    ...
    parser.add_argument("--peft_unfreeze_last_n_blocks", type=int, default=0)
...
def main() -> None:
    ...
    # Keep inference runtime behavior consistent with train.py on this machine:
    # fallback to non-cuDNN conv path when cuDNN engine selection fails.
    if torch.cuda.is_available():
        try:
            rt = ensure_cuda_conv_runtime(allow_cudnn_fallback=True)
            print(
                f"[runtime] cuda probe_ok={rt.get('probe_ok')} "
                f"cudnn_enabled={rt.get('cudnn_enabled')} "
                f"fallback_applied={rt.get('fallback_applied')}"
            )
        except Exception as e:
            print(f"[runtime] probe failed: {e}")
    ...
    def _arg_or_cfg(arg_value, cfg_key, default_value):
        if (arg_value == default_value) and (cfg_key in cfg):
            return cfg[cfg_key]
        return arg_value
    ...
    if args.quality_cuts is not None:
        try:
            ...
            quality_cuts = None
    elif "quality_cuts" in cfg:
        try:
            quality_cuts = sorted([float(v) for v in list(cfg.get("quality_cuts", []))])
        except Exception:
            quality_cuts = None

    clip_model = str(_arg_or_cfg(args.clip_model, "clip_model", CLIP_MODEL))
    patch_pool_tau = float(_arg_or_cfg(args.patch_pool_tau, "patch_pool_tau", float(PATCH_POOL_TAU)))
    patch_pool_mode = str(_arg_or_cfg(args.patch_pool_mode, "patch_pool_mode", str(PATCH_POOL_MODE)))
    patch_trim_p = float(_arg_or_cfg(args.patch_trim_p, "patch_trim_p", float(PATCH_TRIM_P)))
    patch_quality = str(_arg_or_cfg(args.patch_quality, "patch_quality", str(PATCH_QUALITY)))
    patch_pool_gamma = float(_arg_or_cfg(args.patch_pool_gamma, "patch_pool_gamma", float(PATCH_POOL_GAMMA)))
    svd_rank_cap = _arg_or_cfg(args.svd_rank_cap, "svd_rank_cap", SVD_RANK_CAP)
    if svd_rank_cap is not None:
        svd_rank_cap = int(svd_rank_cap)
    adapter_last_n_layers = int(_arg_or_cfg(args.adapter_last_n_layers, "adapter_last_n_layers", 0))
    svd_residual_mode = str(_arg_or_cfg(args.svd_residual_mode, "svd_residual_mode", "add"))
    svd_residual_gate_init = float(_arg_or_cfg(args.svd_residual_gate_init, "svd_residual_gate_init", 0.0))
    svd_residual_train_mode = str(_arg_or_cfg(args.svd_residual_train_mode, "svd_residual_train_mode", "full"))
    peft_unfreeze_last_n_blocks = int(_arg_or_cfg(args.peft_unfreeze_last_n_blocks, "peft_unfreeze_last_n_blocks", 0))
    peft_unfreeze_layernorm = bool(_arg_or_cfg(args.peft_unfreeze_layernorm, "peft_unfreeze_layernorm", True))
    peft_unfreeze_bias = bool(_arg_or_cfg(args.peft_unfreeze_bias, "peft_unfreeze_bias", False))
    normalize_mean = _parse_rgb_triplet(_arg_or_cfg(args.normalize_mean, "normalize_mean", None), CLIP_MEAN)
    normalize_std = [
        max(1e-6, float(v))
        for v in _parse_rgb_triplet(_arg_or_cfg(args.normalize_std, "normalize_std", None), CLIP_STD)
    ]

    multi_expert_enable = bool(_arg_or_cfg(args.multi_expert_enable, "multi_expert_enable", False))
    multi_expert_k = int(_arg_or_cfg(args.multi_expert_k, "multi_expert_k", 1))
    multi_expert_route = str(_arg_or_cfg(args.multi_expert_route, "multi_expert_route", "quality_bin"))
    multi_expert_route_temp = float(_arg_or_cfg(args.multi_expert_route_temp, "multi_expert_route_temp", 1.0))
    multi_expert_route_detach = bool(_arg_or_cfg(args.multi_expert_route_detach, "multi_expert_route_detach", True))
    multi_expert_hybrid_mix = float(_arg_or_cfg(args.multi_expert_hybrid_mix, "multi_expert_hybrid_mix", 0.0))
    ...
    if hasattr(model, "set_patch_scale"):
        model.set_patch_scale(patch_pool_gamma)
    ...
    quality_route_modes = {"quality_bin", "quality_soft", "hybrid", "degrade_hybrid"}
    infer_quality_bin = bool(args.infer_quality_bin)
    if (not infer_quality_bin) and multi_expert_enable and (multi_expert_route.lower() in quality_route_modes) and (quality_cuts is not None):
        infer_quality_bin = True
        print("[info] auto-enabled q_log inference from checkpoint config for quality-based routing.")

    emit_q_log = infer_quality_bin and multi_expert_enable and (multi_expert_route.lower() in quality_route_modes)
    if multi_expert_enable and (multi_expert_route.lower() in quality_route_modes) and (quality_cuts is None):
        print("[warn] quality-based routing active but quality_cuts missing; route may fallback to uniform.")
