...
def _apply_cap_or_trim(x: torch.Tensor, cap_p: Optional[float], trim: bool, required_k: int) -> torch.Tensor:
...
class Trainer:
    ...
    def train_step(self, batch, global_step):
        ...
        with autocast(device_type=self.device.type, enabled=self.use_amp):
            ...
            loss_rank = loss_dict.get("rank_loss", 0.0)
            rank_pairs = loss_dict.get("rank_pairs", 0.0)
            reg_term = torch.tensor(0.0, device=loss.device)
            me_term = torch.tensor(0.0, device=loss.device)
            rank_term = torch.tensor(0.0, device=loss.device)
            rank_scale = 0.0
            ...
            # ---------------- Pairwise ranking loss (optional) ----------------
            rank_enable = bool(getattr(self.opt, "rank_loss_enable", False))
            rank_lambda = float(getattr(self.opt, "rank_loss_lambda", 0.0) or 0.0)
            if rank_enable and (rank_lambda > 0.0):
                rank_scale = _ramp_scale(
                    global_step,
                    getattr(self.opt, "rank_loss_warmup_steps", 0),
                    getattr(self.opt, "rank_loss_ramp_steps", 0),
                )
                rank_f = (
                    loss_rank.float()
                    if hasattr(loss_rank, "float")
                    else torch.tensor(loss_rank, device=loss.device, dtype=torch.float32)
                )
                rank_term = rank_lambda * float(rank_scale) * rank_f
                loss = loss + rank_term
            # -----------------------------------------------------------------
        ...
        if self.use_amp:
            ...
            if self.opt.grad_clip is not None and self.residual_params:
                self.scaler.unscale_(self.optimizer)
                torch.nn.utils.clip_grad_norm_(self.residual_params, float(self.opt.grad_clip))
        ...
        stats = {
            ...
            "loss_rank": float(loss_rank.item()) if hasattr(loss_rank, "item") else float(loss_rank),
            "loss_rank_term": float(rank_term.item()) if hasattr(rank_term, "item") else float(rank_term),
            "rank_scale": float(rank_scale),
            "rank_pairs": float(rank_pairs.item()) if hasattr(rank_pairs, "item") else float(rank_pairs),
