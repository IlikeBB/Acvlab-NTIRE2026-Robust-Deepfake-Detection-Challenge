...
def _normalize_steps(opt, total_steps):
    for name in [
        ...
        "rank_loss_warmup_steps",
        "rank_loss_ramp_steps",
    ]:
...
def train_one_epoch(trainer, loader, global_step, log_every, audit_window):
    ...
    max_steps = int(getattr(trainer.opt, "max_train_steps_per_epoch", 0) or 0)
    for batch_idx, batch in enumerate(iterator, start=1):
        if max_steps > 0 and batch_idx > max_steps:
            logging.info("[train] reached max_train_steps_per_epoch=%d, early stop this epoch.", max_steps)
            break
        if batch is None:
            continue
        ...
        audit_recent.append((batch_stratum, batch_label_qbin, batch_q_values))
        ...
        losses_rank.append(stats.get("loss_rank", 0.0))
        losses_rank_term.append(stats.get("loss_rank_term", 0.0))
        ...
        if log_every and batch_idx % log_every == 0:
            ...
            mean_rank = float(np.mean(losses_rank[-100:])) if losses_rank else 0.0
            mean_rank_term = float(np.mean(losses_rank_term[-100:])) if losses_rank_term else 0.0
            ...
            logging.info(
                ...
                "rank=%.4f rank_term=%.4f patch_evi=%.4f patch_d|mean=%.4f",
                batch_idx,
                mean_cls,
                mean_reg,
                reg_ratio,
            ...
            audit_q_values = []
            for rec_stratum, rec_label_qbin, rec_q_values in audit_recent:
...
def main():
    ...
    for epoch in range(int(opt.niter)):
        ...
        if int(opt.save_epoch_freq) > 0 and epoch % int(opt.save_epoch_freq) == 0:
